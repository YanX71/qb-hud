...
const playerHud = {
  ...
  methods: {
    hudTick(data) {
      ...
      if (this.cinematic == 1) {
        $("#baseplate-container").hide();
        $("#main-container").hide();
      } else if (this.cinematic == 0) {
        $("#baseplate-container").show();
        $("#main-container").show();
      }
      ...
      if (data.talking && data.radio) {
        this.talkingColor = "#d6be00";
      } else if (data.talking) {
        this.talkingColor = '#FFFF3E';
      } else {
        this.talkingColor = "#e3e3e3";
      }
...
function updateFuel(fuel) {
  let minFuel = 0;
  let maxFuel = 100;
  let dashOffsetAtMin = 0;
  let dashOffsetAtMax = 400;
  let dashArray = mapRange(minFuel, maxFuel, dashOffsetAtMin, dashOffsetAtMax, fuel);
  let gauge = document.querySelector('.fuel-gauge .value');
  gauge.style.strokeDasharray = dashArray + " 600";
  saferInnerHTML(document.querySelector('.fuel-text'), Math.round(fuel) + " l");
  lineCapCheck(gauge, fuel);
}

function showFuel(isShown) {
  let fuel = document.querySelector(".fuel-gauge");
  showElement(fuel, isShown);
}

function lineCapCheck(element, value) {
  if (value <= 0) {
...
function updateStat(statName, value, maxValue = 100) {
  let minValue = 0;
  let dashArrayAtMin = 0;
  let dashArrayAtMax = 251;
  let dashArray = mapRange(minValue, maxValue, dashArrayAtMin, dashArrayAtMax, value);
  let gauge = document.querySelector(".stat-gauge." + statName +" .value");
  gauge.style.strokeDasharray = dashArray + " 600";
  lineCapCheck(gauge, value);
}
